...
public class Utils {
	...
	public static String AESEncrypt(String encodeRules, String content) {
		try {
			...
			// 11.将字符串返回
			return HexUtils.bytesToHexString(byte_AES);
	 ...
	 */
	public static String AESDiscrypt(String encodeRules, String content) {
		try {
			// 1.构造密钥生成器，指定为AES算法,不区分大小写
			KeyGenerator keygen = KeyGenerator.getInstance("AES");
			// 2.根据ecnodeRules规则初始化密钥生成器
			...
			byte[] byte_content = HexUtils.hexStringToBytes(content);
			/*
			 * 解密
			 */
